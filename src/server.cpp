...
static void handle_client(int client_fd, sockaddr_in client_addr, fs::path uploads_dir, uint64_t rate_limit_bps) {
  ...
  for (;;) {
    ...
    if (op == "UPLOAD") {
      ...
      iss >> filename >> size >> md5_expected;
      ...
      if (fs::exists(out_path) && fs::is_regular_file(out_path)) {
        ...
        if (existing < size) {
          ...
          std::string md5_now = ft::md5_file(out_path, kChunkSize);
          if (!md5_expected.empty() && md5_now == md5_expected) {
            std::ostringstream done;
            done << "OK AlreadyHave MD5 " << md5_now << " MATCH";
      ...
      {
        std::string md5_now = ft::md5_file(out_path, kChunkSize);
        if (!md5_expected.empty()) {
          std::ostringstream fin;
          fin << "OK Saved MD5 " << md5_now << " " << ((md5_now == md5_expected) ? "MATCH" : "MISMATCH");
          ft::send_frame(client_fd, fin.str());
        } else {
          std::ostringstream fin;
          fin << "OK Saved MD5 " << md5_now;
          ft::send_frame(client_fd, fin.str());
        }
      }
    ...
    if (op == "DOWNLOAD") {
      ...
      std::string md5_now = ft::md5_file(in_path, kChunkSize);
      std::ostringstream resp;
      resp << "OK " << size << " " << md5_now << " OFFSET " << offset;
      if (!ft::send_frame(client_fd, resp.str())) break;
...
int main(int argc, char** argv) {
  ...
  if (argc >= 3) rate_limit_bps = static_cast<uint64_t>(std::stoull(argv[2]));
  ...
  for (;;) {
    ...
    int cfd = ::accept(server_fd, reinterpret_cast<sockaddr*>(&caddr), &clen);
    if (cfd < 0) {
    ...
    std::thread([cfd, caddr, uploads_dir, rate_limit_bps]() mutable {
