...
inline std::string format_upload_cmd(const std::string& filename, uint64_t size, const std::string& md5) {
  std::ostringstream oss;
  oss << "UPLOAD " << filename << " " << size;
  if (!md5.empty()) oss << " " << md5;
  return oss.str();
}
...
inline bool parse_download_ok(const std::string& resp, DownloadOk& out) {
  ...
  std::string ok, kw;
  DownloadOk tmp;
  iss >> ok >> tmp.size >> tmp.md5 >> kw >> tmp.offset;
  if (ok != "OK" || kw != "OFFSET") return false;
  out = tmp;
  return true;
}
